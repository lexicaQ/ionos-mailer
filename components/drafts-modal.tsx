...
export function DraftsModal({
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            toast.success("Entwurf gespeichert (inkl. Dateien)")
        } catch (error: any) {
            console.error(error);
    ...
    const handleLoadDraft = (draft: EmailDraft) => {
        try {
            if (!draft || typeof draft !== 'object') throw new Error("Ungültige Daten")
            onLoadDraft(draft)
            setOpen(false)
        } catch (e: any) {
    ...
    const openSaveDialog = (existingDraft?: EmailDraft) => {
        if (existingDraft) {
            ...
            setDraftName(currentSubject || `Entwurf ${format(new Date(), 'dd.MM.yy HH:mm')}`)
            setSelectedDraftId(null)
        }
    ...
    return (
        <>
                                        ...
                                        >
                                            <div className="flex justify-between items-start gap-4 mb-2">
                                                ...
                                                <Badge variant="outline" className="shrink-0 text-[10px] font-mono bg-neutral-50 dark:bg-[#252525] border-neutral-200 dark:border-neutral-800 text-neutral-500">
                                                    {format(new Date(draft.updatedAt), 'dd.MM', { locale: de })}
                                                </Badge>
                                            </div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-3 pt-3 mt-1 border-t border-neutral-100 dark:border-neutral-800/50">
                                                ...
                                                {attachmentCount > 0 && (
                                                    <div className="flex items-center gap-1.5 text-xs text-blue-600 dark:text-blue-400 font-medium" title={draft.attachments?.map(a => a.name).join(', ')}>
                                                        ...
                                                        {attachmentCount} Datei{attachmentCount !== 1 && 'en'}
                                                    </div>
                                                )}
            ...
            <ResponsiveModal
                ...
                title={selectedDraftId ? "Entwurf aktualisieren" : "Entwurf speichern"}
                description={currentAttachments.length > 0 ? "Anhänge werden mitgespeichert." : "Wählen Sie einen Namen für Ihren Entwurf."}
            ...
            >
                <div className="py-4 space-y-4">
                    {selectedDraftId && (
                        <Badge variant="outline" className="mb-2 bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800">
                            Bestehenden Entwurf überschreiben
                        </Badge>
                    )}
                    ...
                    {currentAttachments.length > 0 && (
                        <span className="flex items-center gap-2 mb-2 text-sm text-blue-500">
                            ...
                            {currentAttachments.length} Anhänge
                        </span>
                    )}
                ...
                <div className="flex justify-end gap-2">
                    <Button variant="ghost" onClick={() => setSaveDialogOpen(false)}>Abbrechen</Button>
