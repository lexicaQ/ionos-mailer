...
export function LiveCampaignTracker() {
    ...
    // SIMPLIFIED: Always fetch fresh from server, no caching
    const fetchCampaigns = useCallback(async () => {
        // Prevent concurrent fetches
        if (isFetching.current) {
            console.log('[LiveTracker] Fetch already in progress, skipping...');
            return;
        }
    ...
    useEffect(() => {
        // Always fetch fresh from server when modal opens
        if (open) {
            fetchCampaigns();
        }
    }, [open, fetchCampaigns]);
                    ...
                    {isLoading ? (
                        ...
                        <>
                            ...
                            {filteredJobs.map((job) => {
                                ...
                                const isOverdue = isPending && diffInMinutes > 2;
