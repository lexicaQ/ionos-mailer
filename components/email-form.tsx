...
export function EmailForm() {
    ...
    // Defined BEFORE auto-save to ensure we check before any new saves overwrite
    useEffect(() => {
                ...
                if (ageMinutes < 1440) {
                    ...
                    useEffect(() => {
                        const subscription = form.watch((value) => {
                            const bodyContent = value.body?.replace(/<[^>]*>/g, '').trim() || '';
                            const hasSubject = !!value.subject?.trim();
                            const hasRecipients = value.recipients && value.recipients.length > 0;

                            // Only save if there's actual meaningful content
                            // This prevents saving empty initial states (e.g. <p></p>) which would overwrite recovery data
                            if (hasSubject || bodyContent.length > 0 || hasRecipients) {
                                const data = {
    ...
    const onSubmit = useCallback(async (data: EmailFormValues) => {
        ...
        try {
            ...
            if (useBackground) {
                ...
                if (typeof window !== 'undefined') {
                    // Dispatch custom event with the campaign data to trigger Optimistic Update in Live Tracker
                    const event = new CustomEvent('campaign-created', {
                        detail: resultData.campaign // Contains the optimistic object we just got from API
                    });
                    window.dispatchEvent(event);

                    window.dispatchEvent(new Event('email-sent')); // Instant sync trigger
                }
